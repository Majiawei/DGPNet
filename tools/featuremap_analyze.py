...
def forward(self, img, img_metas=None, return_loss=False, **kwargs):
    ...
    # outs = self.bbox_head.forward_common(x1, x2)
    # return outs


def create_model(cfg, checkpoint, use_gpu=True):
...
def create_featuremap_vis(cfg, checkpoint, use_gpu=True, init_shape=(320, 320, 3)):
    model = create_model(cfg, checkpoint, use_gpu)
...
def show_featuremap_from_imgs(featurevis, feature_indexs, img_dir, mean, std, is_show, output_dir):
    ...
    img_paths = traverse_file_paths(img_dir, 'png')
...
if __name__ == '__main__':
    ...
    img_dir = 'data/kins/testing/image_2/'
    ...
    use_gpu = False
    is_show = True
    init_shape = (640, 640, 3)  # 值不重要，只要前向一遍网络时候不报错即可
    feature_index = [159, 160, 161, 162, 163] # fpn out  # 想看的特征图层索引(yolov3  218 214 210)
    # feature_index = [175, 203, 231, 259, 287] # cls out
    # feature_index = [185, 213, 241, 269, 297] # reg out
    featurevis = create_featuremap_vis(cfg, checkpoint, use_gpu, init_shape)
    ...
    show_featuremap_from_imgs(featurevis, feature_index, img_dir, mean, std, is_show, output_dir)
