...
class Balancer(nn.Module):
    ...
    def compute_fg_mask(self, gt_boxes2d, shape, downsample_factor=1, device=torch.device("cpu")):
        ...
        for b in range(B):
            gt_boxes2d[b] /= downsample_factor
            gt_boxes2d[b][:, :2] = torch.floor(gt_boxes2d[b][:, :2])
            gt_boxes2d[b][:, 2:] = torch.ceil(gt_boxes2d[b][:, 2:])
            gt_boxes2d[b] = gt_boxes2d[b].long()
            N = gt_boxes2d[b].shape[0]
            for n in range(N):
    ...
    def forward(self, loss, gt_boxes2d):
        ...
        fg_mask = self.compute_fg_mask(gt_boxes2d=gt_boxes2d,
                                       shape=loss.shape,
                                       downsample_factor=self.downsample_factor,  # 1
                                       device=loss.device)
        bg_mask = ~fg_mask
