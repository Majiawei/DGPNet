...
@PIPELINES.register_module()
class NormalizeImgAndDepth(Normalize):
    def __init__(self, mean, std, d_mean, d_std, to_rgb=True):
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
    ...
    def __call__(self, results):
        ...
        results["depth"] = mmcv.imnormalize(results["depth"], self.d_mean, self.d_std,self.to_rgb)
        results['img_norm_cfg'] = dict(mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        results['depth_norm_cfg'] = dict(d_mean=self.d_mean, d_std=self.d_std, to_rgb=self.to_rgb)
        return results 

@PIPELINES.register_module()
class NormalizeImgWithoutDepth(Normalize):
    def __init__(self, mean, std, to_rgb=True):
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        self.to_rgb = to_rgb
    ...
    def __call__(self, results):
        ...
        results['img_norm_cfg'] = dict(mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        # results['depth_norm_cfg'] = dict(d_mean=self.d_mean, d_std=self.d_std, to_rgb=self.to_rgb)
        # results['depth'] = results["depth"]/256.0
        # results['depth'] = skimage.transform.downscale_local_mean(image=results["depth"],
        #                                                    factors=(
        #                                                        self.depth_downsample_factor,
        #                                                        self.depth_downsample_factor))
        return results
@PIPELINES.register_module()
class NormalizeDepth(Normalize):
    def __init__(self, depth_downsample_factor=4):
