...
class Kitti2DDepthDataset(CustomDataset):
    r"""KITTI 2D Dataset.
    ...
    def prepare_train_img(self, idx):
        ...
        depth_raw_info = self.data_infos[idx]['depth']
        img_info = dict(filename=img_raw_info['image_path'])
        depth_info = dict(filename=depth_raw_info['depth_path'])
        ...
        self.pre_pipeline(results) # Prepare results dict for pipeline
    ...
    def bbox2result_kitti2d(self,
        ...
        if pklfile_prefix is not None:
            ...
            pklfile_path = (
                pklfile_prefix if pklfile_prefix.endswith(
                    ('.pkl', '.pickle')) else pklfile_prefix+'.pkl')
    ...
    def evaluate(self, results, metric='bbox', pklfile_prefix=None, eval_types=None):
        ...
        """
        result_files, tmp_dir = self.format_results(results, pklfile_prefix,submission_prefix="./kitti_submission_results_dir/data")
